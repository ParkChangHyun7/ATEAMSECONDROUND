...
public class ChatController {
    ...
    private static final String OLLAMA_INTENT_PROMPT = "You are a machine that performs a single task. Your task is to classify text. You MUST ONLY respond with a JSON object. Do NOT write code. Do NOT write explanations. If the user's message is about weather, respond with: {\"is_weather_query\": true} If the user's message is NOT about weather, respond with: {\"is_weather_query\": false} There are no other possible responses.";
    ...
    public ResponseEntity<String> chat(@RequestBody String userMessage) {
        ...
        try {
            // 날씨 관련 질문인지 먼저 판단 (Ollama에게 직접 물어봄)
            ...
            requestBody.put("temperature", 0.0); // Temperature를 낮춰서 응답 다양성을 줄임
    ...
    private boolean isWeatherIntent(String userMessage) {
        ...
        try {
            ...
            if (ollamaResponse.getStatusCode() == HttpStatus.OK && ollamaResponse.getBody() != null) {
                ...
                // 정규 표현식을 사용하여 ```json ... ``` 블록 추출
                String jsonBlockPattern = "(?s)```json\\s*(\\{.*?\\})\\s*```";
                java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(jsonBlockPattern);
                java.util.regex.Matcher matcher = pattern.matcher(ollamaRawAnswer);
                ...
                if (extractedJson != null && !extractedJson.isEmpty()) {
                    try {
                        ...
                        if (jsonNode.has("is_weather_query")) {
                            result = jsonNode.path("is_weather_query").asBoolean();
                        }
                    } catch (Exception jsonEx) {
                        System.err.println("isWeatherIntent - Error parsing extracted JSON: " + jsonEx.getMessage()); // Error log
                    }
                }
